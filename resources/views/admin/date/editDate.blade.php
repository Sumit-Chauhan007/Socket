                    ...
                    <div class="card mb-4">
                        ...
                        <div class="card-body size_box">
                            <div class="mb-3 col-xl-8">
                                ...
                                <input type="text" class="form-control" value="{{ $event->name }}" id="EventName"
                                    aria-describedby="emailHelp">
                            </div>
                            <div class="mb-3 col-xl-8">
                                <label for="EventPlace" class="form-label">Place</label>
                                <input type="text" class="form-control" id="EventPlace" value="{{ $event->place }}"
                                    aria-describedby="emailHelp">
                            </div>
                            <div class="mb-3 col-xl-8">
                                <label for="EventDescription" class="form-label">Description</label>
                                <input type="text" class="form-control" id="EventDescription"
                                    value="{{ $event->description }}" aria-describedby="emailHelp">
                            </div>
                            <div class="mb-3 col-xl-8">
                                <label for="EventImage" class="form-label">Image</label>
                                <input type="file" class="form-control" id="EventImage"
                                    accept="image/png, image/gif, image/jpeg" aria-describedby="emailHelp">
                            </div>
                            <div class="mb-3 col-xl-8">
                                <label for="EventDate" class="form-label">Date</label>
                                <input type="date" class="form-control" id="EventDate" value="{{ $event->date }}"
                                    aria-describedby="emailHelp">
                            </div>
                            <button type="button" onclick="addEvent('{{ $event->uuid }}')" class="btn btn-success text-white">Save</button>
                        </div>
                    </div>
                </div>
            </div>
                    ...
                    if ($.isEmptyObject(data.error)) {
                        ...
                        $.each(data.error, function(field_name, error) {
                            $(document).find('#' + field_name + '').closest('div').after(
                                '<span class="text-strong text-danger errorSpan">' + error +
                                '</span>')
